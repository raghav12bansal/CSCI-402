...
/* Faber test function declaration */
 ...
 */
static void *bootstrap(int arg1, void *arg2) {
  /* If the next line is removed/altered in your submission, 20 points will be
   * deducted. */
  dbgq(DBG_TEST,
       "SIGNATURE: "
       "53616c7465645f5fcd2c76c4f41b5f191f52d26bb37a4acf557ab30caee75d6cc86bfd3"
       "f29f61721019348dd9c45e441\n");
 ...
 * the process commonly refered to as the "init" process, which should have
 * PID 1.
 *
...
static kthread_t *initproc_create(void) {
  ...
  init_thread = kthread_create(init_process, initproc_run, NULL, NULL);
...
int my_sunghan_test(kshell_t *kshell, int argc, char **argv) {
  proc_t *child_process = proc_create("sughan_test");
  kthread_t *child_thread =
      kthread_create(child_process, sunghan_test, 0, NULL);
  sched_make_runnable(child_thread);
  int status;
  do_waitpid(child_process->p_pid, 0, &status);
  return 0;
}
int my_sunghan_deadlock_test(kshell_t *kshell, int argc, char **argv) {
  proc_t *child_process = proc_create("sughan_deadlock_test");
  kthread_t *child_thread =
      kthread_create(child_process, sunghan_deadlock_test, 0, NULL);
  sched_make_runnable(child_thread);
  int status;
  do_waitpid(child_process->p_pid, 0, &status);
  return 0;
...
int my_faber_thread_test(kshell_t *kshell, int argc, char **argv) {
  proc_t *child_process = proc_create("faber_test");
  kthread_t *child_thread =
      kthread_create(child_process, faber_thread_test, 0, NULL);
  sched_make_runnable(child_thread);
  int status;
  do_waitpid(child_process->p_pid, 0, &status);
  return 0;
...
static void *initproc_run(int arg1, void *arg2) {
  ...
  kshell_add_command("deadlock", my_sunghan_deadlock_test,
                     "Run sunghan_deadlock_test().");
