...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
        ...
        char* kernel_stack = alloc_stack();

        //allocating memory for the new thread
        kthread_t *new_thread= slab_obj_alloc(kthread_allocator);

        new_thread->kt_kstack = kernel_stack;
        new_thread->kt_retval = (void *)0;
        ...
        //p_threads list initialised in process create so not initialising here.
        list_insert_head(&p->p_threads, &new_thread->kt_plink);        

        //thread context initialisation 
        context_setup(&new_thread->kt_ctx, func, arg1, arg2, kernel_stack, DEFAULT_STACK_SIZE, p->p_pagedir);
...
kthread_cancel(kthread_t *kthr, void *retval)
{
        ...
        if (kthr == curthr) {
                ...
                panic("Kthread exit will never return!\n");

        }
        else {
                //thread is sleeping
                //check if thread is in cancellable sleep or not
                
                if (kthr->kt_state == KT_SLEEP_CANCELLABLE) {
                        //Then you must wake up thread Y
                        //When thread Y starts executing in the CPU the next time, it should be executing code inside a cancellation point (that you have implemented). 
                        //In this case, you need to make sure that these functions return the specified error condition to indicate that this "error condition" has occurred so that the thread code can execute correctly.
                        //wake up thread and make it runnable
                        
                        ktqueue_remove(kthr->kt_wchan, kthr); //removing thread from wait q
                        sched_make_runnable(kthr); //adding kthr to run q
                        dbg(DBG_PRINT, "(GRADING1C)\n");                       
                }
...
kthread_exit(void *retval)
{
        ...
        panic("Kthread exit function will never reach here!\n");
        return;
}
